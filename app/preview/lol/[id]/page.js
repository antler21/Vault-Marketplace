...
.rv-rank-history-row { display: flex; gap: 5px; overflow-x: auto; padding-bottom: 4px; }
.rv-rank-history-row::-webkit-scrollbar { height: 4px; }
.rv-rank-history-row::-webkit-scrollbar-thumb { background: #2a3a4a; border-radius: 2px; }
.rv-season-card { flex: 0 0 auto; background: #0d1520; border: 1px solid rgba(200,155,60,0.1); padding: 6px 10px; min-width: 52px; display: flex; flex-direction: column; gap: 2px; }
.rv-season-label { font-family: 'Cinzel', serif; font-size: 9px; color: #3c4a5c; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 2px; }
.rv-season-split { font-family: 'Inter', sans-serif; font-size: 8px; color: #3c4a5c; margin-bottom: 1px; }
.rv-season-rank { font-family: 'Inter', sans-serif; font-size: 11px; font-weight: 700; }
          ...
          {tab === 'OVERVIEW' && (
            <div className="rv-overview">
              ...
              {scan.rank_history && scan.rank_history.length > 0 && (
                <>
                  <div className="rv-overview-section">Season Rank History</div>
                  <div className="rv-rank-history-row">
                    {scan.rank_history.map(s => (
                      <div key={s.season} className="rv-season-card">
                        <div className="rv-season-label">S{s.season}</div>
                        {s.splits.map((sp, i) => (
                          <div key={i}>
                            {sp.split != null && <div className="rv-season-split">Split {sp.split}</div>}
                            <div className="rv-season-rank" style={{ color: sp.rank ? (RANK_COLORS[sp.rank] || '#e8e0d0') : '#2a3a4a' }}>
